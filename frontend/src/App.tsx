...
export default function App() {
  ...
  const handleSendResponse = async () => {
    ...
    try {
      const response = await fetch("https://gigacorp-ai-backend.vercel.app/api/v1/support/chat", {
        method: "POST",
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const response = await fetch("https://gigacorp-ai-backend.vercel.app/api/v1/support/upload", {
        method: "POST",
        body: formData
      });
